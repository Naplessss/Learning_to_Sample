...
def parse_args(config_path):
    ...
    parser.add_argument('--sampler', type=str, default='rw-my',
                        choices=['rw-my', 'ns', 'node-my', 'edge', 'node', 'cluster'])
    parser.add_argument('--gcn_type', type=str, default='sage', choices=['sage', 'gat'])
    parser.add_argument('--use_gpu', type=int, default=1, choices=[0, 1])
    parser.add_argument('--save_log', type=int, default=1, choices=[0, 1])
    parser.add_argument('--save_summary', type=int, default=1, choices=[0, 1])
    ...
    parser.add_argument('--meta_learning_rate', type=float, default=2e-3)
    parser.add_argument('--drop_out', type=float, default=0.2)
    parser.add_argument('--meta_drop_out', type=float, default=0.2)
    parser.add_argument('--meta_sampler_type', type=str, default='prob', 
                        choices=['prob','bernouli','hard','normalized','none'])
...
def get_log_name(args, prefix='test', use_args=None):
    if use_args is None:
        use_args = ['dataset', 'batch_size', 'train_sample', 'eval_sample', 'loss_norm', 'sampler', 'gcn_type','meta_sampler_type']
    args = vars(args)
